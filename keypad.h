...
enum key_t {stop=0,none=1,dopler=2,up=3,down=4,left=5,right=6,shock=7,unknown=8};
...
class Keypad_T{
 public:   
  ...
  static void setOnKeyChanged( onKeyChanged_t someFunction);
 ...
 private:
  ...
  static const uint32_t DIMMERINTERVAL = 172800000; // =1000*60*60*24*2; // it was 20000; //ms 
 
  inline static uint32_t actionTimestamp = millis();
  inline static uint32_t scanTimestamp = 0;

  inline static key_t lastKeypressed = none;
  inline static key_t history[historySize] = {none};
...
template<class T,class R> void Keypad_T<T,R>::powerOn(){
  ...
  if (!isPowered()){
   R::init(); // Rfid itself will decide if it's connected or not
  }
  powered = true;
...
template<class T,class R> bool Keypad_T<T,R>::isSpellMatched(const key_t* sample ){
  ...
  for (uint8_t i = 0; i < historySize-1; i++ ){
    key_t x = pgm_read_byte_near( &sample[i] ); 
    if (x == stop) {
...
template<class T,class R> void Keypad_T<T,R>::poll(){
 ...
 key_t justKeypressed = none;   
 uint32_t savedMillis = millis(); // careful: it's perishable!

 if ((savedMillis-actionTimestamp)>DIMMERINTERVAL){
  powerOff();
 }
 
 if ((savedMillis-actionTimestamp)>SPELLINTERVAL){
   pushIntoHistory(none); // spoil the spell
 }
 ...
 if ((savedMillis-scanTimestamp >= SCANINTERVAL)and(enabled)){
  ...
  switch ( analogValue ){
     case T::bttnNoneLo ... T::bttnNoneHi:     justKeypressed = none;   break;
     case T::bttnDoplerLo ... T::bttnDoplerHi: 
      justKeypressed = T::doplerConnected() ? dopler : none; 
      break;
     case T::bttnDownLo ... T::bttnDownHi:     justKeypressed = down;   break;
     case T::bttnUpLo ... T::bttnUpHi:         justKeypressed = up;     break;
     case T::bttnLeftLo ... T::bttnLeftHi:     justKeypressed = left;   break;
     case T::bttnRightLo ... T::bttnRightHi:   justKeypressed = right;  break;
     case T::bttnShockLo ... T::bttnShockHi:   justKeypressed = shock;  break;      
     default: justKeypressed = unknown;
  } // switch 
  
  if (justKeypressed != lastKeypressed) {
   lastKeypressed = justKeypressed; // m/b we can use history for it?
   actionTimestamp = savedMillis;
   if (justKeypressed != none) { // TODO : m/b we should check it first??? p.s. with unknown
    
    // we have to keep "dopler" out of history to let spells work
    if ((justKeypressed != dopler)&&(justKeypressed != unknown)) { 
     ...
     if (spellBook[i].SpellPtr != nullptr) {
       if ( isSpellMatched(spellBook[i].SpellPtr) ) {   
         if ( spellBook[i].SpellHandlerPtr != nullptr ) {
             ...
             for (uint8_t j=0; j<historySize; j++) history[j] = none; // clear history 
         }
         break;
       }
     }  
...
template<class T,class R> void Keypad_T<T,R>::setOnKeyChanged( onKeyChanged_t someFunction ){
  onKeyChangedPtr = someFunction;
}
